...
tensorflow_dirs = {}
pytorch_commands = {
        'ConvNets' : '\"python ./multiproc.py --nproc_per_node {gpus} ./main.py --arch {model} -b {batchs} --training-only -p 10 --prof 1 --epochs 1 --data-backend pytorch /tmp/ILSVRC2012/\"',
        'imagenet' : '\"python main.py -a {model} --epochs 1 -b {batchs} -j {gpus} --multiprocessing-distributed --rank=0 /home/shared/ILSVRC2012\"',
        #'imagenet' : '\"python main.py -a {model} --epochs 1 -b {batchs}  --rank=0 /home/shared/ILSVRC2012\"',
        'efficient_det': '\"python train_prof.py --dataset VOC --dataset_root /home/shared/VOCdevkit/ --network {model} --batch_size {batchs} --iter 100 --wramup 30\"',
...
if args.ncu:
    ...
    if args.batchs > 0:
        result_dir = result_dir + '/' + args.mode + '/' + args.model + '/' +args.model+'gpu-'+ str(args.gpus)+'batchs-'+str(args.batchs) 
        os.makedirs(result_dir, exist_ok=True)
        sbatch_command = 'sbatch sbatch_gpu'+ str(args.gpus) + '.sh ' + command.format(model=args.model, batchs=args.batchs, gpus=args.gpus) + ' \"'+ working_dir  + '\" \"' + result_dir + '\"'
        os.system(sbatch_command)
    else:
        batchsize=16
        for i in range(4):
            ...
            os.makedirs(result_dir, exist_ok=True)
            sbatch_command = 'sbatch sbatch_gpu'+ str(args.gpus) + '.sh ' + command.format(model=args.model, batchs=batchsize, gpus=args.gpus) + ' \"'+ working_dir  + '\" \"' + result_dir + '\"'
            os.system(sbatch_command)    
            batchsize*=2
